...
Процедура РасчитатьОтпуск()
	...
	Если КоличествоРабочихДней = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Данные по отпускам не могут быть расчитаны, заполните рафик работы");
		Возврат;
	КонецЕсли;
...
Функция ТекстЗапроса_СформироватьДвиженияОсновныеНачисленияОтпуск()
